...
from . import StaticPubExtension
...
class ExportJob(object):
    ...
    def __init__(self, export):
        self.export = export
        self.url_opener = self.create_url_opener()
    ...
    def create_url_opener(self):
        url_opener = urllib2.build_opener()
        url_opener.addheaders = [
            ("User-agent", StaticPubExtension.USER_AGENT),
            ("x-woost-extensions-staticpub-export", str(self.export.id))
        ]
        return url_opener
...
class ExportedResource(object):
    ...
    def open(self, url_opener):

        response = url_opener.open(self.source_url)
