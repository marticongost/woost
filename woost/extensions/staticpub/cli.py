...
class CLI(object):
    ...
    def create_arg_parser(self):
        ...
        def destination_parser(id):
            try:
                ...
                raise ArgumentTypeError("%r is not a valid destination ID" % id)
    ...
    def _track_job_progress(self, job):
        ...
        def show_execution_summary(e):
            progress_bar.finish()
            self.end = time()
            print "Processed %d publications and %d dependencies in %.2fs" % (
                tasks_count,
        ...
        def task_starting(e):
            ...
            if e.task["language"]:
                progress_bar.label += (
                    u" (%s)" % translate_locale(e.task["language"])
                )
        ...
        def task_successful(e):
            progress_bar.label += u" => exported"
            progress_bar.update(1)

        @when(job.task_failed)
        def task_failed(e):
            progress_bar.label += u" => failed (%s)" % e.task["error_message"]
            progress_bar.update(1)
        ...
        def resource_transfer_failed(e):
            progress_bar.label += u" => failed (%s)" % e.error
            progress_bar.update(1)
        ...
        for action in sorted(tasks):
            ...
            for cls, cls_tasks in tasks[action].iteritems():
                ...
                for publishable, languages in cls_tasks.iteritems():
                    print ("%d" % publishable.id).ljust(10),
                    print styled(
                        translations(
                            publishable,
